...
internal static class CollectionShape
{
    ...
    /// Arrays go through the element parser's <c>CreateTypedArray</c> with a
    /// real-element-type fallback (<see cref="CommandReflectionCache.CreateTypedArray(Type, int)"/>)
    /// when no parser exists for the element type (never <c>object[]</c>);
    ...
    internal static object? Create(Type propertyType, Type elementType, IReadOnlyList<object?> converted, ICommandTypeParserCollection typeParsers)
    {
        ...
        if (kind == CollectionKind.Array)
        {
            ...
            if (typeParsers.TypeParsers.TryGetValue(elementType, out var parser))
            {
                ...
                {
                    array = CommandReflectionCache.CreateTypedArray(elementType, converted.Count);
                }
            }
            else
            {
