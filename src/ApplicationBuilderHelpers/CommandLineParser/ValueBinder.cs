    ...
    public void SetCommandValues(ParseResult result)
    {
            ...
            StringComparer.OrdinalIgnoreCase))
        {
                ...
                && elementType is not null)
            {
                ...
                propertyValue = CollectionShape.Create(option.PropertyType, elementType, converted, typeParserCollection, displayName);
            }
            else
            {
        ...
        foreach (var (argument, values) in result.ArgumentValues)
        {
                ...
                && elementType is not null)
            {
                ...
                propertyValue = CollectionShape.Create(argument.PropertyType, elementType, converted, typeParserCollection, displayName);
            }
            else
            {
                string? raw = values[0];
