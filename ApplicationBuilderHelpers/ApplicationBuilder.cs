    ...
    public async Task<int> RunAsync(string[] args)
    {
        ...
        foreach (var command in _commands)
        {
            if (command.Name == null || string.IsNullOrEmpty(command.Name))
            {
                ...
                WireHandler(rootHierarchy, command, cancellationTokenSource.Token);
            }
            else
            {
                ApplicationCommandHierarchy? currentChildHier = rootHierarchy;
                foreach (var splitCommand in command.Name!.Split(' '))
                {
                    Command childCommand = new(splitCommand, command.Description);
                    if (!currentChildHier.SubCommands.TryGetValue(splitCommand, out var subHier))
                    {
                        subHier = new(splitCommand, childCommand);
                        currentChildHier.Command.AddCommand(childCommand);
                        currentChildHier.SubCommands.Add(splitCommand, subHier);
                    }
                    currentChildHier = subHier;
                }
                currentChildHier.AppCommand = command;
                WireHandler(currentChildHier, currentChildHier.AppCommand, cancellationTokenSource.Token);
            }
        }
