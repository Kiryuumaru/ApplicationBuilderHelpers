...
| `["bogus", "--help"]` | `2` | `No command found for 'bogus'` (`UnknownCommand` — zero-match wins before help; skipped when the root itself has an implementation plus a position-0 argument, in which case the token binds as the root positional) |
