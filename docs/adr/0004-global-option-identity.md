...
- Keys are case-sensitive throughout: canonical-key dedup/merge uses `StringComparison.Ordinal`/`StringComparer.Ordinal` (`ParseResult.cs:24,37,73`; key def at `:59-60`) and `StringComparer.Ordinal` grouping (`ValueBinder.cs:32-35`); long/short token matching is `Ordinal` (`SubCommandOptionInfo.cs:368,377,382,388,392,405,414,423,425,435`, `FindNoValueBase` at `:362-369`).
- Precedence is pinned: scalar repeats are last-wins — `AddOptionValue` evicts prior canonical-key entries before appending (`ParseResult.cs:31-43`); collections accumulate and bind from the merged list (`ValueBinder.cs:43-54`); merged values decide CLI-wins via `TryGetMergedOptionValues` (`ParseResult.cs:67-77`); environment-variable fallback applies only when no merged CLI value exists and the env value is not null-or-whitespace (`EnvVarFallback.cs:29-35`), applied per `AllOptions` scope before binding (`ValueBinder.cs:25-28`).
