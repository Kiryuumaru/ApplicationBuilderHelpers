...
- A bare valued option on an abstract prefix reports `MissingRequired` (exit 2) instead of `RequiresSubcommand` (#549 gate at `ArgumentParser.cs`, call after the #508 scan; scratch `ParseOptionsAndArguments` + `ParameterValidator.CollectRequiredErrors` over the pre-`--` slice). Bare long (`--data`), bare short (`-d`), combined cluster tail (`-vd`), and env-set bare (`--cfg` with `MATRIX_CFG` set) all fail `Missing value for option: <display-name>` (required: `Missing required option:`) even with env set — typing claims ownership. Valid valued forms (`--data=x`, `-dvalue`, `--data x`, `--data -5`, `--verbose`, `--no-verbose`), surplus positionals (stay silent so near-miss `RequiresSubcommand` + DidYouMean is preserved), post-`--` tokens, and help/version neighbors fall through unchanged. Pinned by `AbstractRootBareOptionTests.cs`.
