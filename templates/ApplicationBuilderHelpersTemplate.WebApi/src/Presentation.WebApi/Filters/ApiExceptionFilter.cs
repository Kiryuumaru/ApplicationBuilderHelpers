...
internal sealed class ApiExceptionFilter(ProblemDetailsFactory problemDetailsFactory, ILogger<ApiExceptionFilter> logger) : IAsyncExceptionFilter
{
    public Task OnExceptionAsync(ExceptionContext context)
    {
        ...
        if (TryMapException(exception, out var statusCode, out var title, out var detail, out var extensions))
        {
            if (statusCode >= 500)
            {
                logger.LogError(exception, "Unhandled exception mapped to {StatusCode}: {Title}", statusCode, title);
            }
            else
            {
                logger.LogInformation("Exception mapped to {StatusCode}: {Title}", statusCode, title);
            }
        ...
        logger.LogError(exception, "Unhandled exception (unmapped) caught by ApiExceptionFilter");

        var unexpectedProblem = problemDetailsFactory.CreateProblemDetails(
            httpContext,
