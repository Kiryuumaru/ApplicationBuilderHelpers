...
internal sealed class DefaultLogEnricher(IConfiguration configuration, Dictionary<string, object?>? scopeMap) : ILogEventEnricher
{
    ...
    private static void EnrichLogSourceContext(LogEvent evt)
    {
        ...
        {
            if (prop.Key.StartsWith(LoggerExtensions.SourceContextActionIdentifierAndSeparator))
            {
                keysToRemove.Add(prop.Key);
                var split = prop.Key.Split(LoggerExtensions.SourceContextActionSeparator);
                if (split.Length != 3)
                {
                    continue;
                }
        ...
        {
            evt.AddOrUpdateProperty(new LogEventProperty(LoggerExtensions.SourceContextActionsIdentifier, new SequenceValue(sourceContexts)));
        }

        if (lastSourceContextAction != null && currentSourceContext == lastSourceContext)
        {
            AddProperty(evt, LoggerExtensions.SourceContextActionIdentifier, lastSourceContextAction, addAndReplace: true);
        }
    }
