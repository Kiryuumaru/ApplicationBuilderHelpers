...
{
    private static readonly TimeSpan ReadLineTimeout = TimeSpan.FromMilliseconds(500);
    private static readonly TimeSpan LogPollingInterval = TimeSpan.FromMilliseconds(10);
    private static readonly TimeSpan FileListenerPollingDelay = TimeSpan.FromSeconds(5);
        ...
        try
        {
            ...
            async Task printLogEventTail(int count, CancellationToken cancellationToken)
            {
                ...
                while (true)
                {
                    ...
                    foreach (var line in (await streamReader.ReadToEndAsync(cancellationToken)).Split(Environment.NewLine).Reverse())
                    {
                        ...
                        catch { /* Malformed log line; skip and continue */ }
                        if (logEvent != null)
                        {
            ...
            await LatestFileListener(async logFile =>
            {
                ...
                if (previousCts != null)
                {
                    ...
                    catch { /* Already cancelled or disposed; safe to ignore */ }
                    finally
                    {
                        previousCts.Dispose();
                    }
                ...
                try
                {
                    ...
                    if (!hasPrintedTail)
                    {
                        try
                        {
                            while (!ct.IsCancellationRequested)
                            {
                                try
                                {
                                    var line = await streamReader.ReadLineAsync(ct.WithTimeout(ReadLineTimeout));
                        ...
                        catch { /* Seek-to-tail failed; start reading from current position */ }
                        hasPrintedTail = true;
                    }
                    ...
                    while (!ct.IsCancellationRequested)
                    {
                        ...
                        catch { /* Malformed log line or cancellation; skip and continue */ }
                        if (logEvent != null)
                        {
                            stampLog(logEvent);
                        ...
                        {
                            await Task.Delay(LogPollingInterval, ct);
                        }
                    }
                }
                catch { /* Log file stream closed or cancelled; listener will retry with next file */ }
            }, cancellationToken);
        }
        finally
        {
    ...
    private async Task LatestFileListener(Action<AbsolutePath> onLogfileChanged, CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            await Task.Delay(FileListenerPollingDelay, cancellationToken);
        }
    }
    ...
    private Task<AbsolutePath?> GetLatestLogFile(AbsolutePath[] skipLogFiles, CancellationToken cancellationToken)
    {
        ...
        return Task.Run(() =>
        {
            ...
            foreach (var logFile in configuration.GetLogsPath().GetFiles())
            {
                try
                {
                ...
                catch { /* Invalid log filename format; skip this file */ }
            }
            if (latestLogTime.LogStr == null)
            {
                return null;
            }
            return AbsolutePath.Create(configuration.GetLogsPath() / $"log-{latestLogTime.LogStr}.jsonl");
        }, cancellationToken);
    }
}
