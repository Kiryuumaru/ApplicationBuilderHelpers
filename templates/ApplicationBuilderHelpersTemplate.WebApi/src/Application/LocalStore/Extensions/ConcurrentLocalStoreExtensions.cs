...
public static class ConcurrentLocalStoreExtensions
{
    ...
    /// </summary>
    public static Task SetAsync(this IConcurrentLocalStore store, string id, bool value, CancellationToken cancellationToken = default)
        => store.SetAsync(id, value ? bool.TrueString : bool.FalseString, cancellationToken);
    ...
    /// </summary>
    public static Task SetAsync<T>(this IConcurrentLocalStore store, string id, T value, CancellationToken cancellationToken = default)
        where T : IFormattable
    {
        var format = GetFormat(value);
        var formatted = value.ToString(format, CultureInfo.InvariantCulture);
        return store.SetAsync(id, formatted, cancellationToken);
    }
    ...
    /// </summary>
    public static async Task<T?> GetAsync<T>(this IConcurrentLocalStore store, string id, CancellationToken cancellationToken = default)
        where T : IParsable<T>
    {
    ...
    /// </summary>
    public static async Task<T?> GetAsync<T>(this IConcurrentLocalStore store, string id, JsonTypeInfo<T?> jsonTypeInfo, CancellationToken cancellationToken = default)
        where T : class
    {
    ...
    /// </summary>
    public static Task SetAsync<T>(this IConcurrentLocalStore store, string id, T? obj, JsonTypeInfo<T?> jsonTypeInfo, CancellationToken cancellationToken = default)
        where T : class
    {
        var data = JsonSerializer.Serialize(obj, jsonTypeInfo);
        return store.SetAsync(id, data, cancellationToken);
    }
