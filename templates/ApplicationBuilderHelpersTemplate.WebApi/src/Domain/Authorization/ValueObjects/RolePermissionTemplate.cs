...
/// <summary>
/// Template for dynamically resolving role permission identifiers with parameters.
/// </summary>
public sealed class RolePermissionTemplate : ValueObject
{
    ...
    private static string NormalizeTemplate(string template)
    {
        ...
        if (!TemplatePatterns.PlaceholderRegex.IsMatch(trimmed))
        {
    ...
    private static IReadOnlyCollection<string> ExtractPlaceholders(string template)
    {
        ...
        var matches = TemplatePatterns.PlaceholderRegex.Matches(template);
        if (matches.Count == 0)
        {
            return Array.Empty<string>();
        }
