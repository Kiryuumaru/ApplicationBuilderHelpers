...
/// </summary>
public sealed class TokenExpirationTests(ITestOutputHelper output) : WebApiTestBase(output)
{
    ...
    public async Task RefreshToken_AfterAllSessionsRevoked_Returns401()
    {
        ...
        await RegisterUserAsync(username);
        var firstLogin = await LoginUserWithResponseAsync(username);
        var secondLogin = await LoginUserWithResponseAsync(username);
    ...
    public async Task NewTokens_HaveDifferentValues_EachTime()
    {
        ...
        var auth2 = await LoginUserWithResponseAsync(username);
        var auth3 = await LoginUserWithResponseAsync(username);
    ...
    public async Task TokenType_IsBearerForAllResponses()
    {
        ...
        var loginResponse = await LoginUserWithResponseAsync(username);
