...
The abstract-root precedence: #512 reserved help-word misuse, then #542 invalid flag literals, then the #508 unknown-option scan, then the #549 bare-valued gate — each reporting `InvalidValue`/`UnknownOption`/`MissingRequired` (exit 2) before the `RequiresSubcommand` throw at `ArgumentParser.cs:78`. Valid literals, bare flags, valid valued forms, unknown bases (still `UnknownOption`), surplus positionals (silent, preserving near-miss `RequiresSubcommand` + DidYouMean), `--no-`-prefixed tokens, and post-`--` tokens fall through unchanged.
